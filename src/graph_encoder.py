...
class GraphConvNet(nn.Module):
    ...
    def __init__(self, node_input_dim=36, edge_input_dim=10, hidden_dim=128, 
        ...
        # Attention layer for node-level importance
    ...
    def forward(self, batch):
        """
        Forward pass for batch from molecular graphs
        """
        x, edge_index, edge_attr, batch_idx = batch.x, batch.edge_index, batch.edge_attr, batch.batch
        
        #DEBUG 1: raw node features
        if not torch.isfinite(x).all():
            log_debug("[GCN_X_INPUT_NAN] node features contain NaNs or Infs")
        ...
        if not torch.isfinite(x).all():
            log_debug("[GCN_X_EMBED_NAN] after node_embedding+ReLU got NaNs")
        ...
        if not torch.isfinite(graph_repr).all():
            log_debug("[GCN_GRAPH_REPR_NAN] after graph_norm got NaNs")
        ...
        if not torch.isfinite(output).all():
            log_debug("[GCN_OUTPUT_NAN] final GCN output has NaNs")
            raise RuntimeError("GCN output has NaNs")
        
        return output

class MolecularGraphEncoder(nn.Module):
    """
    Complete encoder that combines SMILES parsing with GCN
    """
    
    def __init__(self, output_dim=256, hidden_dim=128, num_layers=3, dropout=0.1):
        super(MolecularGraphEncoder, self).__init__()
        
        self.graph_converter = MolecularGraph()
        
        # Calculation of feature dimentionality
    ...
    def forward(self, smiles_list):
        ...
        if len(graphs) == 0:
            # Fallback in case there are not valid molecules
            batch_size = len(smiles_list)
            output_dim = self.gcn.output_dim
            return torch.zeros(batch_size, output_dim, device=device), list(range(batch_size))
        ...
        if not torch.isfinite(molecular_representations).all():
            ...
            raise RuntimeError("GCN internal output (per-graph) has NaNs")

        
        # Recreate the whole batch with zeros for invalid molecules
        batch_size = len(smiles_list)
        output_dim = molecular_representations.shape[1]
        full_output = torch.zeros(batch_size, output_dim, device=device)
...
def test_graph_encoder():
    ...
    try:
        ...
        print("✅ Graph Encoder test succsessful!")
        return True
        
    except Exception as e:
        print(f"❌ Error in Graph Encoder test: {e}")
        import traceback
        traceback.print_exc()
        return False
