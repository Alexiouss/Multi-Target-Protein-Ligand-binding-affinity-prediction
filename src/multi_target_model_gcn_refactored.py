...
class MultiTargetPredictor(nn.Module):
    ...
    def __init__(self, config_path="config.json", model_type="quantum",target_names_override=None):
        ...
        self.target_names = target_names_override if target_names_override is not None else list(self.config["targets"].keys())
        ...
        # Attention mechanism (quantum or classical)
        ...
        # Shared output activation and scaling to pKd range [3, 12]        
    ...
    def forward(self, smiles_list):
        ...
        if not torch.isfinite(molecular_features).all():
            log_debug(
                f"[GCN_OUT_NAN] min={molecular_features.min().item()} "
